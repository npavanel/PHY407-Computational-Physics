...
#create an array of temperatures of length n
T_f = 0.5
T_steps = 0.5
T_i = 10
T_array = np.arange(T_f, T_i, T_steps)[:-1]
...
#empty arrays for mean and stdev values
Emean=np.zeros(len(T_array))
stdev=np.zeros(len(T_array))
    ...
    for j in range(n):
        ...
        if monomer_array[new_monomer_pos[0], new_monomer_pos[1]] == 0:
            ...
            if distance_okay:
                ...
                if random() < np.exp(-(new_energy-energy)/T_array[k]):
                    # make switch
                    energy = new_energy
                    monomer_coords = np.copy(new_monomer_coords)
                    monomer_array = np.copy(new_monomer_array)
    Emean[k]=np.mean(energy_array)
    stdev[k]=np.std(energy_array)
#%%
plt.title("Energy vs Temperature")    
plt.errorbar( T_array,Emean, yerr=stdev, label="Error", ecolor="red")
plt.ylabel("Temperature")
plt.xlabel("Energy")
plt.legend()
'''
plt.figure()
plt.title('T_array, N=2e6')
...
plt.title('T_array, Energy = {1:.1f}'.format(T_array, energy))
# plot monomers
for i in range(N):
